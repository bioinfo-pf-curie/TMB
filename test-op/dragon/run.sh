...
do
    SAMPLE=$(basename $i)
    VCF=/data/tmp/egirard/dragon_tests/${RUN}/ANALYSIS/${SAMPLE}/CALLING/${SAMPLE}.hg19_multianno.vcf
    REC=/data/tmp/egirard/dragon_tests/${RUN}/ANALYSIS/RESULTS/VARIANTS/${RUN}_table_report_tagged_tmb_final.tsv
    ...
    # TMB1: 
    cmd="time python ../../bin/pyTMB.py -i ${IVCF} --filterIndels --minVAF 5 ${filters} ${configs}"
    ...
    # TMB2:
    cmd="time python ../../bin/pyTMB.py -i ${IVCF} --minVAF 5 ${filters} ${configs}"
    ...
    # TMB3
    cmd="time python ../../bin/pyTMB.py -i ${IVCF} --filterIndels --minVAF 10 ${filters} ${configs}"
    ...
    # TMB4
    cmd="time python ../../bin/pyTMB.py -i ${IVCF} --minVAF 10 ${filters} ${configs}"
